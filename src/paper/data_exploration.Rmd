...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message= FALSE,fig.width = 12,fig.height = 6,fig.align = 'center',dpi=300,dev='png',fig.retina = 2, fig.cap = NULL)
```
...
```{r series-length-vs-rating-episodes, echo=FALSE, warning=FALSE, message=FALSE,fig.cap="",fig.width=12,fig.height=6, dpi=300}
# Scatter plot for count of episodes vs average rating (cleaned data)
library(ggplot2)

ggplot(cleaned_data, aes(x = episode_count, y = averageRating)) +
  geom_jitter(alpha = 0.1, size = 1.2, width = 0.5, height = 0.5, color ="skyblue") +  
  geom_smooth(method="lm", color ="darkred", se=FALSE, size=1.5)+
...
```{r total-years-vs-rating, echo=FALSE, warning=FALSE, message=FALSE,fig.cap="",fig.width=12,fig.height=6,dpi=300}
...
ggplot(cleaned_data, aes(x = total_years, y = averageRating)) +
  geom_jitter(alpha = 0.1, size = 1.5, width = 0.1, height = 0.1, color = "darkgreen") +  
